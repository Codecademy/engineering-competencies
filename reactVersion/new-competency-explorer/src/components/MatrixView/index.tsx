...
export const MatrixView: React.FC = () => {
    ...
    for (let cat of categories) {
        children.push(<MatrixLabel key={`matrixlabel-${cat}`} label={cat} />)
    }

    for (let l of levels) {
        children.push(<MatrixLabel key={`matrixlabel-${l}`} label={l} />);
    ...
    return (
        <div 
            ...
            style={{
                gridTemplateColumns: `minmax(2rem, 8rem) repeat(${categories.length}, 1fr)`
            }}
        >
            {children.map((c) => c)}
        </div>
