...
export const renderCompetency = (competency: ICompetency, levelGroup: string) => {
    ...
    const out = createElement({
        cls: 'competency',
        children: [
            { content: competency.name, cls: ['name', otherLevels.length === 0 ? 'bold' : ''] },
            otherLevels.length !== 0 ? { cls: 'levels', children: 
            [
                { content: 'Also in:' }, 
                { type: 'ul', 
